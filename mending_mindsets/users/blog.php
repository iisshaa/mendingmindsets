...
if($conn) 
{
 ...
 if($username == '' ||  $topic=='' || $title=='' || $content=='')
    {
        echo "Incomplete credentials";
        exit;
    }
    else{
            $query="INSERT INTO table2 VALUES(NULL,'$username','$topic','$title','$content')";
            $exec_query=mysqli_query($conn,$query);
            if($exec_query==true)
            {
                echo '<script type="text/javascript">

                window.onload = function () { alert("Blog submitted successfully!!"); }
